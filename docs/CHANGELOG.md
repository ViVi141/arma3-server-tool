...
- **数据库优化**：批量插入玩家信息，显著提升监控性能。
- **超时控制**：进程验证添加 2 秒超时，避免 UI 阻塞。
...
- **托盘提示**：首次最小化到系统托盘时显示气球提示，改善功能发现性。
- **状态优化**：简化 UUID 显示，状态栏更简洁。

### 代码质量
- **功能开关**：`PerformanceFeatures` 类支持性能优化的受控发布。
- **性能监控**：扩展 `UiPerformanceProbe` 支持详细性能指标追踪。
